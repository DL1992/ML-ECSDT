...
class ECSDT(object):
    ...
    def fit(self,X,y,cost_mat):
        ...
        for i in range(self.num_estimators):
            S,features,target,costs,s_oob,target_oob,costs_oob=self.sample(X,y,cost_mat,bootstrap,bootstrap_features)
            classifier = cost_tree.CostSensitiveDecisionTreeClassifier(max_features=self.max_features,pruned=self.pruned)
    ...
    def create_stacking_matrix(self,X):
        ...
        for estimator in range(valid_estimators):
            X_stacking[:, estimator] = self.models[valid_estimators[estimator]].predict(X[:, self.features_drawn[valid_estimators[estimator]]])
        return X_stacking
    ...
    def sample(self,X,y,cost_mat,bootstrap,bootstrap_features):
        ...
        return S,selected_features, y[indices], cost_mat[indices, :] ,(X[oob_indices])[:, selected_features], y[oob_indices], cost_mat[oob_indices, :]
